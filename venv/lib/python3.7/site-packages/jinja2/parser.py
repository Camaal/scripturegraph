...
class Parser(object):
    ...
    def parse_import_context(self, node, default):
        ...
        ) and self.stream.test("name:context"):
    ...
    def parse_include(self):
        ...
        node.template = self.parse_expression()
        if self.stream.current.test("name:ignore") and self.stream.test(
            "name:missing"
        ):
    ...
    def parse_compare(self):
        ...
        while 1:
            ...
            elif self.stream.current.test("name:not") and self.stream.test(
                "name:in"
            ):
                self.stream.skip(2)
